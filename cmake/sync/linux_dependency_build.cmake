...
# Builds the locked Linux dependency closure completely offline through the
...
if(NOT DEFINED RF_OPERATION OR NOT RF_OPERATION STREQUAL "dependencies")
    message(FATAL_ERROR "RF1582 Linux dependency build accepts only RF_OPERATION=dependencies")
endif()
...
include("${RF_REPOSITORY_ROOT}/cmake/sync/linux_dependencies.cmake")
...
rf_prepare_linux_dependency_authority("${RF_REPOSITORY_ROOT}")
rf_seed_linux_vcpkg_downloads("${RF_REPOSITORY_ROOT}")
rf_seed_linux_vcpkg_tool_trees("${RF_REPOSITORY_ROOT}")
...
# Prepared roots come first and in dependency order. The two system directories
# at the end carry only the locked host closure the unix OpenSSL recipe requires,
# `perl` and GNU `make`; no component is admitted merely by being on PATH.
set(path_entries "")
foreach(tool_directory IN ITEMS "tools/llvm/bin" "tools/cmake/bin" "tools/ninja")
    list(APPEND path_entries "${prepared_root}/${tool_directory}")
endforeach()
list(APPEND path_entries "/usr/bin" "/bin")
list(JOIN path_entries ":" build_path)

set(vcpkg_executable "${prepared_root}/vcpkg/vcpkg")
...
execute_process(
    COMMAND "${CMAKE_COMMAND}" -E env
        ...
        --triplet x64-linux-rawframe
        --host-triplet x64-linux-rawframe
...
if(NOT install_result EQUAL 0)
    ...
    rf_bootstrap_fail("RF1583" "offline vcpkg dependency build failed: ${install_diagnostic}")
endif()

set(installed_triplet "${prepared_root}/installed.incoming/x64-linux-rawframe")
foreach(required IN ITEMS
        ...
        "lib/libgtest.a"
        "lib/libgmock.a"
        "lib/manual-link/libgtest_main.a"
        "lib/manual-link/libgmock_main.a"
        "lib/libcrypto.a"
        "lib/libssl.a")
    if(NOT EXISTS "${installed_triplet}/${required}")
        rf_bootstrap_fail("RF1584" "installed dependency closure is missing ${required}")
    endif()
    file(SIZE "${installed_triplet}/${required}" required_bytes)
    if(required_bytes EQUAL 0)
        rf_bootstrap_fail("RF1584" "installed dependency artifact is empty: ${required}")
    endif()
endforeach()
if(IS_DIRECTORY "${installed_triplet}/debug")
    rf_bootstrap_fail("RF1584" "release-only closure unexpectedly contains a debug tree")
endif()
if(NOT EXISTS "${prepared_root}/installed.incoming/vcpkg/status")
    rf_bootstrap_fail("RF1584" "vcpkg installation database is missing")
endif()
...
if(EXISTS "${installed_previous}")
    rf_bootstrap_fail("RF1585" "stale installed-closure rollback state exists")
endif()
if(EXISTS "${installed_destination}")
    ...
    if(NOT stage_result STREQUAL "0")
        rf_bootstrap_fail("RF1586" "cannot stage existing installed closure for replacement")
    endif()
endif()
...
if(NOT publish_result STREQUAL "0")
    ...
    rf_bootstrap_fail("RF1586" "cannot atomically publish the installed dependency closure")
endif()
...
message(STATUS "RF1587 Linux dependency closure built offline and published")
