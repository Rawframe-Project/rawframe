...
// The shipped command, run with its output captured.
//
// Not through `runBoundedProcess`: that runner caps a job at one active process
// because the thing it exists to run, the schema oracle, is a leaf. The shipped
// command is not a leaf, and driving it through a runner built for leaves would
// be testing the harness rather than the tool. The cap is right where it is.
struct ShippedRun {
    int exitCode = -1;
    std::string output;
};
...
ShippedRun runShipped(const std::vector<std::string>& arguments) {
    std::string line = "\"" + std::filesystem::path(RAWFRAME_TEST_RF_EVIDENCE).string() + "\"";
    for (const auto& kArgument : arguments) {
        line += " \"" + kArgument + "\"";
    }
    line += " 2>&1";
...
#if defined(_WIN32)
    run.exitCode = _pclose(pipe);
#else
    const int kStatus = pclose(pipe);
    run.exitCode = WIFEXITED(kStatus) ? WEXITSTATUS(kStatus) : -1;
#endif
    return run;
}
...
//
// Receipts go into the repository's own store, a set is assembled from a
// declared plan, that set is evaluated, and every link is validated. Everything
// it produces lands under out/, which is generated output and is never
// committed; the chain is evidence that the four authorities compose, not an
// artifact to keep.
TEST(Conformance, TheCompleteTier0ChainRunsThroughTheShippedCommand) {
    const auto kRoot = caseRoot();
    ASSERT_TRUE(std::filesystem::exists(std::filesystem::path(RAWFRAME_TEST_RF_EVIDENCE)));
    const std::string kRepository = repositoryRoot().generic_string();

    // Link one: every receipt the plan names is put into the store by content.
    for (const std::string_view kReceipt : {std::string_view{"evaluable-1"}, std::string_view{"evaluable-2"}}) {
        const auto kPut =
            runShipped({"put",
                        "blob",
                        "--root",
                        kRepository,
                        "--source",
                        "tools/rf_evidence/tests/fixtures/evidence/receipts/" + std::string(kReceipt) + ".json",
                        "--media",
                        std::string(kRawRunReceiptMediaType)});
        EXPECT_EQ(kPut.exitCode, 0) << kPut.output;
        EXPECT_NE(kPut.output.find(R"("digest":"sha256:)"), std::string::npos) << kPut.output;
    }

    // Link two: the ledger, assembled from the plan and never enumerated.
    const auto kAssemble = runShipped({"assemble",
                                       "evidence-set",
                                       "--root",
                                       kRepository,
                                       "--plan",
                                       "tools/rf_evidence/tests/fixtures/evidence/plans/evaluable-pass.json",
                                       "--set-id",
                                       "dddddddd-eeee-4fff-8000-000000000001"});
    ASSERT_EQ(kAssemble.exitCode, 0) << kAssemble.output;
    const std::filesystem::path kSetPath = kRoot / "assembled-set.json";
    {
        std::ofstream output(kSetPath, std::ios::binary | std::ios::trunc);
        output << kAssemble.output;
    }
    const auto kValidateSet =
        runShipped({"validate", "record", "--root", kRepository, "--record", kSetPath.generic_string()});
    EXPECT_EQ(kValidateSet.exitCode, 0) << kValidateSet.output;

    // Link three: the verdict, reached from the stored receipts and the
    // maintained authorities, and validated as a record in its own right.
    const auto kEvaluate = runShipped({"evaluate",
                                       "evidence-set",
                                       "--root",
                                       kRepository,
                                       "--set",
                                       kSetPath.generic_string(),
                                       "--evaluation-id",
                                       "dddddddd-eeee-4fff-8000-000000000002"});
    ASSERT_EQ(kEvaluate.exitCode, 0) << kEvaluate.output;
    EXPECT_NE(kEvaluate.output.find(R"("outcome":"passed")"), std::string::npos);
    const std::filesystem::path kReceiptPath = kRoot / "evaluation-receipt.json";
    {
        std::ofstream output(kReceiptPath, std::ios::binary | std::ios::trunc);
        output << kEvaluate.output;
    }
    const auto kValidateReceipt =
        runShipped({"validate", "record", "--root", kRepository, "--record", kReceiptPath.generic_string()});
    EXPECT_EQ(kValidateReceipt.exitCode, 0) << kValidateReceipt.output;

    // Link four: the promotion pointer, which validates and promotes nothing.
    const auto kValidateBaseline = runShipped({"validate",
                                               "record",
                                               "--root",
                                               kRepository,
                                               "--record",
                                               "tools/rf_evidence/tests/fixtures/evidence/baselines/well-formed.json"});
    EXPECT_EQ(kValidateBaseline.exitCode, 0) << kValidateBaseline.output;
    ...
    const auto kIndex = runShipped({"load", "evidence-index", "--root", kRepository});
    EXPECT_EQ(kIndex.exitCode, 0) << kIndex.output;
    EXPECT_EQ(kIndex.output.find("baseline"), std::string::npos) << kIndex.output;
}
...
// operation with the invalid-arguments exit code, which is what a caller
// scripting a promotion would actually hit.
TEST(Conformance, NoCommandLineVerbCanPromoteActivateSignOrUploadAnything) {
    const std::string kRepository = repositoryRoot().generic_string();
    for (const std::vector<std::string>& kVerb : {std::vector<std::string>{"promote", "baseline"},
                                                  std::vector<std::string>{"activate", "baseline"},
                                                  std::vector<std::string>{"revoke", "baseline"},
                                                  std::vector<std::string>{"emit", "baseline-record"},
                                                  std::vector<std::string>{"sign", "record"},
                                                  std::vector<std::string>{"attest", "record"},
                                                  std::vector<std::string>{"upload", "blob"},
                                                  std::vector<std::string>{"trust", "evidence-set"}}) {
        std::vector<std::string> arguments = kVerb;
        arguments.emplace_back("--root");
        arguments.push_back(kRepository);
        const auto kResult = runShipped(arguments);
        EXPECT_EQ(kResult.exitCode, 2) << kVerb.front() << " " << kVerb.back() << ": " << kResult.output;
        EXPECT_NE(kResult.output.find("unsupported operation"), std::string::npos)
            << kVerb.front() << ": " << kResult.output;
    }
}

// The trust ceiling per route. An input member is refused by the schema const;
// these are the other two ways a caller could try. An unknown argument is
// refused rather than ignored, and an environment variable changes nothing,
// because the tool reads no ambient authority at all.
TEST(Conformance, NeitherAnArgumentNorAnEnvironmentVariableCanRaiseTrustOrTier) {
    const std::string kRepository = repositoryRoot().generic_string();
    const std::string kBaseline = "tools/rf_evidence/tests/fixtures/evidence/baselines/well-formed.json";
    for (const std::vector<std::string>& kAttempt : {std::vector<std::string>{"--trust", "trusted_ci"},
                                                     std::vector<std::string>{"--tier", "tier_2"},
                                                     std::vector<std::string>{"--provenance", "trusted_ci"},
                                                     std::vector<std::string>{"--runner-qualified", "true"},
                                                     std::vector<std::string>{"--baseline-role", "anchor"}}) {
        std::vector<std::string> arguments{"validate", "record", "--root", kRepository, "--record", kBaseline};
        arguments.insert(arguments.end(), kAttempt.begin(), kAttempt.end());
        const auto kResult = runShipped(arguments);
        EXPECT_EQ(kResult.exitCode, 2) << kAttempt.front() << ": " << kResult.output;
    }

    // And the ambient route. The variables are set in this process, so the
    // shipped command inherits them; a tool that read any of them would emit
    // something different from the run below, which it must not.
    const std::vector<std::string> kArguments{"validate", "record", "--root", kRepository, "--record", kBaseline};
    const auto kClean = runShipped(kArguments);
    ASSERT_EQ(kClean.exitCode, 0) << kClean.output;
    for (const std::string_view kName : {std::string_view{"RF_EVIDENCE_TRUST"},
                                         std::string_view{"RF_EVIDENCE_TIER"},
                                         std::string_view{"RAWFRAME_EVIDENCE_PROVENANCE"},
                                         std::string_view{"RAWFRAME_TRUSTED_CI"}}) {
        setEnvironmentVariable(kName, "trusted_ci");
    }
    const auto kAmbient = runShipped(kArguments);
    EXPECT_EQ(kAmbient.exitCode, 0) << kAmbient.output;
    EXPECT_EQ(kAmbient.output, kClean.output) << "an environment variable changed the tool's answer";
    for (const std::string_view kName : {std::string_view{"RF_EVIDENCE_TRUST"},
                                         std::string_view{"RF_EVIDENCE_TIER"},
                                         std::string_view{"RAWFRAME_EVIDENCE_PROVENANCE"},
                                         std::string_view{"RAWFRAME_TRUSTED_CI"}}) {
        setEnvironmentVariable(kName, "");
    }
}
